...
const CompanyCertifications = ({ companyId }) => {
  ...
  const fetchCertifications = async (filterParams = {}) => {
    try {
      ...
      const certificationsRes = await axios.get(
        `${BASE_URL}/api/company-certifications?${queryParams.toString()}`
      );
      ...
      if (certificationsRes.data.success) {
        ...
        throw new Error(
          certificationsRes.data.message || "Failed to load certifications"
        );
      }
    } catch (certError) {
  ...
  const handleDeleteCertification = async (certId) => {
    ...
    try {
      const response = await axios.delete(
        `${BASE_URL}/api/company-certifications/${certId}`
      );
