...
const TrainingList = () => {
  ...
  const fetchTrainings = useCallback(async (page = 1) => {
    try {
      ...
      const response = await axios.get(`${BASE_URL}/api/trainings`, { params });
  ...
  }, [searchTerm, selectedType, selectedField, selectedMethod]);
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setState(prev => ({
          ...prev,
          trainingTypes: typesRes.data.data,
  ...
  // Fetch trainings when filters change
  useEffect(() => {
  ...
  const EmptyState = () => (
    <div className="bg-white p-8 rounded-lg shadow text-center">
      ...
      <h3 className="mt-2 text-lg font-medium text-gray-900">No trainings found</h3>
