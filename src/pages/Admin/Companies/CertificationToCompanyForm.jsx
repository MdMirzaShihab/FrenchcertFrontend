...
import { 
  FaCertificate, FaCalendarAlt, FaClipboardCheck, 
  FaInfoCircle, FaSpinner, FaSave, FaArrowLeft
} from 'react-icons/fa';
import { BASE_URL } from "../../../secrets";

const CertificationToCompanyForm = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // If editing, fetch the certification details
        if (isEditMode) {
          const certResponse = await axios.get(`${BASE_URL}/api/company-certifications/${certificationId}`);
    ...
    fetchInitialData();
  }, [companyId, certificationId, isEditMode]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!formData.certification || !formData.issueDate || !formData.expiryDate) {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(`Certification ${isEditMode ? 'updated' : 'added'} successfully`);
        // Redirect back to company view
        navigate(`/admin/companies/${companyId}`);
      } else {
        throw new Error(response.data.message || `Failed to ${isEditMode ? 'update' : 'add'} certification`);
      }
    } catch (error) {
      console.error('Submit error:', error);
      toast.error(error.response?.data?.message || error.message || `Failed to ${isEditMode ? 'update' : 'add'} certification`);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex justify-between items-center mb-6 pb-4 border-b">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {isEditMode ? 'Update certification details for' : 'Add a new certification to'} {companyDetails?.name}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
            />
            <p className="text-xs text-gray-500 mt-1">Optional surveillance audit date</p>
          </div>
